...
function drawScoreboard() {
	...
	if (gameState === 0) {
		...
		renderText(trueCanvas.width / 2 + gdx + 6 * settings.scale, trueCanvas.height / 2.1 + gdy - 155 * settings.scale, 150, "#2c3e50", "CHILL");
		renderText(trueCanvas.width / 2 + gdx + 5 * settings.scale, h + 10, fontSize, "rgb(44,62,80)", 'Play this mini game  ');
		renderText(trueCanvas.width / 2 + gdx + 5 * settings.scale, h + 50, fontSize, "rgb(44,62,80)", ' until the loading is done');

	} else if (gameState != 0 && textOpacity > 0) {
		textOpacity -= 0.05;
		renderText(trueCanvas.width / 2 + gdx + 6 * settings.scale, trueCanvas.height / 2 + gdy, 60, "rgb(236, 240, 241)", String.fromCharCode("0xf04b"), 'px FontAwesome');
		renderText(trueCanvas.width / 2 + gdx + 6 * settings.scale, trueCanvas.height / 2 + gdy - 155 * settings.scale, 150, "#2c3e50", "CHILL");
		renderText(trueCanvas.width / 2 + gdx + 5 * settings.scale, h, fontSize, "rgb(44,62,80)", 'Play this mini game ');
		renderText(trueCanvas.width / 2 + gdx + 5 * settings.scale, h + 50, fontSize, "rgb(44,62,80)", ' until the loading is done');
...
function setMainMenu() {
	...
	if ($("#pauseBtn").replace(/^.*[\\\/]/, '') == "btn_pause.svg") {
		$("#pauseBtn").attr("src","./images/btn_resume.gif");
	} else {
